...
`   */
    public String query_cache(String command){
        ...
        String Myquery = "select * from queryNew_"+config.db_table_name+ " where id = ?";
...
//                myConn.close();
    ...
    public void add_Entry(String entry) {
        ...
        try{
            String query = " insert into queryNew_"+ config.db_table_name+ " (id, command, resultHash, result)"
                    + " values (?, ?, ? ,?)";
