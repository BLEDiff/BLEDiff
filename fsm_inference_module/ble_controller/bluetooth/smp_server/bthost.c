			   ...
			   const uint8_t *bdaddr, uint8_t addr_type)
{
	...
	struct smp_conn *smp_data = conn->smp_data;
	memcpy(smp_data->tk, bthost->pin, bthost->pin_len); // Set pin number
...
void bthost_request_public_key(struct bthost *bthost, uint16_t handle)
{
    PySys_WriteStdout("In bthost_request_public_key bthost.c \n ");
    struct btconn *conn;
    ...
    //{
        ...
        if (bthost->sc)
            auth_req |= 0x08;
        smp_send_public_key(conn->smp_data, bthost->io_capability, auth_req);
    //}
}

void bthost_request_dh_check(struct bthost *bthost, uint16_t handle)
{
    PySys_WriteStdout("In bthost_request_dh_check bthost.c \n ");
    struct btconn *conn;
    ...
    //if (conn->addr_type == BDADDR_BREDR)
    //{
    smp_send_dh_key_check(conn->smp_data);
    //}
}

void bthost_request_sign_info(struct bthost *bthost, uint16_t handle)
{
    struct btconn *conn;

    conn = bthost_find_conn(bthost, handle);
    if (!conn) {
        PySys_WriteStdout("Culprit bthost.c \n ");
        return;
    }

    //if (conn->addr_type == BDADDR_BREDR)
    //{
    PySys_WriteStdout("Before bthost_request_dh_check \n ");
    smp_send_sign_info(conn->smp_data);
    //}
}


void bthost_request_sm_random(struct bthost *bthost, uint16_t handle)
{
    PySys_WriteStdout("In bthost_request_sm_random bthost.c \n ");
    struct btconn *conn;
    ...
    //if (conn->addr_type == BDADDR_BREDR)
    //{
    PySys_WriteStdout("Before smp_send_sm_random \n ");
    smp_send_sm_random(conn->smp_data);
    //}
}


void bthost_request_pair_confirm(struct bthost *bthost, uint16_t handle)
{
    PySys_WriteStdout("In bthost_request_pair_confirm bthost.c \n ");
    struct btconn *conn;
    ...
    //if (conn->addr_type == BDADDR_BREDR)
    //{
    PySys_WriteStdout("Before smp_send_pair_confirm \n ");
    smp_send_sm_pair_confirm(conn->smp_data);
    //}
}
