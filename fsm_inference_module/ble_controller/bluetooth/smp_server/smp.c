...
static bool send_public_key(struct smp_conn *conn)
{
	...
	// bt_crypto_f4(conn->smp->crypto, conn->local_pk, conn->remote_pk,
	// 			 conn->prnd, 0, buf);
...
static void distribute_keys(struct smp_conn *conn)
{
	...
	{
		// Encryption key + EDIV + Rand
		// TODO: Allow to randomize or set encryption key and master ID
...
int force_local_key_distribution = -1;
...
static void pairing_rsp(struct smp_conn *conn, const void *data, uint16_t len)
{
	...
	if (force_local_key_distribution != -1)
	{
		conn->local_key_dist = force_local_key_distribution;
	}
...
static void pairing_cfm(struct smp_conn *conn, const void *data, uint16_t len)
{
	...
	if (conn->out)
	{
		...
		// TODO: Proper random value
		//auto crypto = bt_crypto_new();
		//bt_crypto_random_bytes(crypto, rsp, 16);
		//bt_crypto_unref(crypto);
		//memcpy(conn->rrnd, rsp, 16);
...
void smp_send_sm_random(void *conn_data)
{
    uint8_t rsp[16];
...
void smp_send_sm_pair_confirm(void *conn_data)
{
    uint8_t rsp[16];

    struct smp_conn *conn = conn_data;
    memset(rsp, 0, 16*sizeof(rsp[0]));
...
void smp_pair(void *conn_data, uint8_t io_cap, uint8_t auth_req)
{
	...
	req.init_key_dist = 0xf; //key_dist(conn->smp->bthost);
	req.resp_key_dist = 0xf; //key_dist(conn->smp->bthost);
...
void smp_send_public_key(void *conn_data, uint8_t io_cap, uint8_t auth_req)
{
...
void smp_send_dh_key_check(void *conn_data)
{
...
void smp_send_sign_info(void *conn_data)
{
				   ...
				   const uint8_t *ra, uint8_t addr_type, bool conn_init)
{
	...
	conn->ia_type = LE_PUBLIC_ADDRESS; // TODO: receive master address type

	if (addr_type == BDADDR_LE_PUBLIC)
		addr_type = 0;
	else if (addr_type == BDADDR_LE_RANDOM)
		addr_type = 1;

	conn->ra_type = addr_type;
	memcpy(conn->ia, ia, 6);
	memcpy(conn->ra, ra, 6);
