					 ...
					 struct channel_info *info)
{
	...
	printk("Wifi: Selected channel: %d", rf->channel);
	if (rf->channel <= 14) {
				      ...
				      struct rt2x00lib_conf *libconf)
{
	...
	printk("Retry limits: short - %d, long - %d", libconf->conf->short_frame_max_tx_count, libconf->conf->long_frame_max_tx_count);
}
