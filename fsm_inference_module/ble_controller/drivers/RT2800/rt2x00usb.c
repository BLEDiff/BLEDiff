							 ...
							 const int timeout)
{
	...
	int status;
	unsigned int pipe =
		(requesttype == USB_VENDOR_REQUEST_IN) ? usb_rcvctrlpipe(usb_dev, 0) : usb_sndctrlpipe(usb_dev, 0);
...
static void rt2x00usb_interrupt_rxdone(struct urb *urb)
{
	...
	/*
	 * Schedule the work for reading the RX status
	 * from the device.
	 */
...
void rt2x00usb_disconnect(struct usb_interface *usb_intf)
{
	 ...
	 */
	rt2x00dev->ops->lib->netlink_remove_device(); // Remove device from netlink device, so other device can be registered again
