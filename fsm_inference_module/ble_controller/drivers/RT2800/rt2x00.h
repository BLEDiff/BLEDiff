...
#define rt2x00_info(dev, fmt, ...)                   \
	wiphy_info((dev)->hw->wiphy, "%s: Infos - " fmt, \
			   __func__, ##__VA_ARGS__)
...
#define CONFIG_UPDATE_BSSID (1 << 3)
	...
	void (*netlink_rx)(struct rt2x00_dev *rt2x00dev, struct sk_buff *skb); // Add fast netlink receiving

	void (*netlink_remove_device)(void); // Add fast netlink receiving
};
								 ...
								 const u16 rt, const u16 rev)
{
	...
	//pr_emerg("My Debugger is Printk\n");
