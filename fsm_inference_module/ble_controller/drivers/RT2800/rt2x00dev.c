									 ...
									 bool success)
{
	...
	retry_rates = test_bit(TXDONE_FALLBACK, &txdesc->flags) ? (txdesc->retry + 1) : 1;
									  ...
									  struct rxdone_entry_desc *rxdesc)
{
	 ...
	 * we can exit now. */
	//printk("size:%d, control:%d\n", skb->len, hdr->frame_control);
...
void rt2x00lib_rxdone(struct queue_entry *entry, gfp_t gfp)
{
	...
	//printk("size:%d, data:%02x\n",rxdesc.size, ((u8 *)entry->skb->data)[0]);
