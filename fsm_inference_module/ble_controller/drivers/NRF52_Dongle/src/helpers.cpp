...
void bytes_to_hex(uint8_t *dst_array, uint8_t *src_array, uint16_t src_len)
{
    ...
    uint8_t low_nibble;
    uint8_t high_nibble;
    for (uint16_t i = 0, j = 0; i < src_len; i += 1, j += 2)
    {
        c = src_array[i];
        low_nibble = lowNibble(c);
        high_nibble = highNibble(c);

        dst_array[j] = (high_nibble < 0xA ? high_nibble + '0' : (high_nibble - 0xA) + 'A');
        dst_array[j + 1] = (low_nibble < 0xA ? low_nibble + '0' : (low_nibble - 0xA) + 'A');
    }
}
