...
from scapy.fields import BitField, ByteField, XByteField, \
    ShortField, LongField, IntField, XIntField, ByteEnumField, StrLenField, XStrField, \
    XStrLenField, XStrFixedLenField, LenField, FieldLenField, FieldListField, \
    PacketField, PacketListField, ConditionalField, PadField, StrFixedLenField
...
class EAPOL_KEY(Packet):
    name = "EAPOL-Key"
    fields_desc = [
        ByteField("key_descryptor_type", 0x02),
        ShortField("key_information", 0x00),
        ShortField("key_length", 32),
        LongField("replay_counter", 0),
        StrFixedLenField("wpa_key_nonce", '\x00' * 32, length=32),
        StrFixedLenField("key_iv", '\x00' * 16, length=16),
        StrFixedLenField("key_rsc", '\x00' * 8, length=8),
        StrFixedLenField("key_id", '\x00' * 8, length=8),
        StrFixedLenField("key_mic", '\x00' * 16, length=16),
        LenField("key_data_len", None, "H"),
        StrLenField("key_data", '', length_from=lambda pkt: pkt.key_data_len),
...
class EAP_MD5(EAP):
    ...
    fields_desc = [
        ...
        XStrLenField("optional_name", '',
                     length_from=lambda p: 0 if p.len is None or p.value_size is None else (p.len - p.value_size - 6))
        # noqa: E501
    ]
